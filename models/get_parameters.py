from models import Autoformer,DLinear, LLM_Linear,PatchTST,MICN,FiLM,TimesNet,Nonstationary_Transformer
import random
import argparse
import torch
import numpy as np
...
parser.add_argument('--AutoCon_wnorm', type=str, default='Mean', help='Window Normalization Techniques: {ReVIN | Mean | LastVal | Decomp}')
# data loader
...
parser.add_argument('--wo_freq', type=str, default=' ',
                        help='freq for time features encoding, options:[s:secondly, t:minutely, h:hourly, d:daily, b:business days, w:weekly, m:monthly], you can also use more detailed freq like 15min or 3h')
...
if __name__ == '__main__':
    model_dict = {
        ...
        'Lla_Linear':LLM_Linear,
    }
    for model in model_dict.keys():
        print(model)
        try:
            count_parameters(model_dict[model].Model(args).float())
        except:
            pass