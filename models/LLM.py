...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.prompt_blank_dict = {
            'ett': str(open('./data/prompt_bank/ETT.txt',encoding='utf-8').read()),
            'ele': str(open('./data/prompt_bank/ECL.txt',encoding='utf-8').read()),
            'tra': str(open('./data/prompt_bank/Traffic.txt',encoding='utf-8').read()),
            'wea': str(open('./data/prompt_bank/Weather.txt',encoding='utf-8').read()),
            'exc': str(open('./data/prompt_bank/Exchange.txt',encoding='utf-8').read()),
            'ill': str(open('./data/prompt_bank/National_illness.txt',encoding='utf-8').read()),
        }
        self.prompt_blank = self.prompt_blank_dict[configs.data_path[:3].lower()]
        try:
    ...
    def forecast(self, x_enc,  x_dec):        
        ...
        # x_dec = self.normalize_layers(x_dec, 'norm')
        # print('x_enc',x_enc)
        B, T, N = x_enc.size()
        ...
        prompt_blank = self.prompt_blank
        prompt = []
        for b in range(x_enc.shape[0]):
            ...
            prompt_ = (
                ...
                "Advice:these data are normalized so the variation is very small,you can inference the trend of next steps across the previous steps, and calculate the special numerical value."
                "Let's think step by step. <|<end_prompt>|>"
            )
